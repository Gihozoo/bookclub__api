...
// view member borrow
export async function viewmemberborrow(req, res) {
    try {
        let member = await Borrow.findOne({where: {member_id: req.params.id}});
        if (member) {
            res.json({
                success: true,
                message: 'borrow records returned successfully',
                data: member
            })
        } else {
            res.json({
